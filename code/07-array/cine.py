...
# Cette fonction efface tout ce qu'il y a dans le terminal (console)
def clear_console():
...
def main():
    ...
    while True: 
        ...
        print("\n\n[<]-(Precedent) [>]-(Suivant)      [Esc]-(Quitter)")
        key = keyboard.read_key() # Mémorise la touche tappé (lorsqu'on appui sur la touche)
        keyboard.read_key()       # Ignore le soulèvement du doigt

        if key == "left" or key == "gauche":    # Touche <<Flèche gauche>> appuyée => Appel à rotate_left()
            rotate_left(FILMS)
            display(FILMS[0])
        elif key == "right" or key == "droite": # Touche <<Flèche droite>> appuyée => Appel à rotate_right()
            rotate_right(FILMS)
            display(FILMS[0])
        elif key == "esc":                      # Touche <<Esc>> appuyée => Quitte la boucle
